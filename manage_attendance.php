...
<div class="container-fluid">
	...
	<form action="" id="employee-attendance">
		<div class="row form-group">
			<div class="col-md-4">
				...
				<select id="employee_id" class="borwser-default select2">
					<option value=""></option>
					<?php 
					$qry="select id,lastname,firstname,employee_no from employee where company_id='$company_id'";
					$data=mysqli_query($conn,$qry);
					while($row=mysqli_fetch_array($data)){
					?>
						<option value="<?php echo $row['id'] ?>"><?php echo $row['lastname'] . ' '. $row['firstname']  . ' | '. $row['employee_no'] ?></option>
					<?php } ?>
				</select>
			</div>
		...
		<div class="form-group">
			...
			<input type="text" name="company_id" id="company_id" required="required" class="form-control text-right" step="any" value="<?php echo isset($company_id) ? $company_id : "" ?>" />
				</div>
...
<script>
	...
	$('#add_list').click(function(){
		...
		tr.find('[name="company_id[]"]').val(company_id)
		...
		tr.find('.company_id').html(company_id)
